...
const Course = ({ data }: { data: Course }) => {
  return (
    <div className=" rounded-lg grid grid-cols-[30%_70%] p-2 space-x-2 bg-[radial-gradient(circle,_#FFFFFF00,_#F4EAD8)]">
      <iframe src={`${data.pdfUrl}#toolbar=0&navpanes=0&scrollbar=0&page=1&view=FitH`} className="rounded-lg border-0" width="100%" height="100%" style={{ border: "none", overflow: "hidden" }}></iframe>
      <div className="space-y-2 ">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="flex justify-center items-center border rounded-full bg-white px-1">
            ...
            <p className="text-[10px]">{data.subject.replace("_", " ")}</p>
          </div>
        </div>
      </div>
    </div>
